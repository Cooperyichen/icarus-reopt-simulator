...
class Port:
    ...
    def __init__(
        ...
        element_id: int = None,
        src_coords: tuple = (0, 0),
        dst_coords: tuple = (0, 0),
        debug: bool = False,
        parent_switch=None
    ):
        self.store = simpy.Store(env)
        self.rate = rate
        self.obp_rate = obp_rate
        self.env = env
        self.out = None
    ...
    def update(self, packet):
        ...
        if self.debug:
            print(f"Retrieved Packet {packet.id} from flow {packet.flow_id}.")
    ...
    def run(self):
        ...
        while True:
            ...
            # #self.parent_switch.failed = True
            # if self.parent_switch.failed:
            #     self.packets_dropped += 1
            #     packet.status = "dropped"
            #     self.dropped_by_flow[packet.flow_id] += 1
            #     print(f"Packet {packet.id} dropped because switch {self.parent_switch.element_id} is failed at time {self.env.now}")
    ...
    def put(self, packet):
        ...
        if self.limit_bytes and byte_count >= self.qlimit:
            ...
            if self.debug:
                print(
                    f"Packet dropped: flow id = {packet.flow_id} and packet id = {packet.id}"
                )
