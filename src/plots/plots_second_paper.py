...
# -*- coding: utf-8 -*-
"""
This script generates plots for the second paper, specifically focusing on the average delay by capacity 
and network Packet Loss Indicator (PLI) by capacity and priority. It uses the Matplotlib and Seaborn libraries for 
...
Functions:
    plot_average_delay__capacity_priority: Plots the average delay versus capacity and priority for different traffic demands.
    plot_average_PLI__capacity_priority: Plots the network PLI versus capacity and priority for different traffic demands.
...
################################################################################################
# def plot_average_delay__capacity_priority(all_results, time_unit='s', plot_priority_our_model=True, plot_priority_capacity=True, plot_overall_our_model=True, plot_overall_capacity=True, results_dir=None):
...
#         model_key = 'our_model' if 'our_model' in scenario else 'capacity'
#         if model_key == 'capacity':
...
#             x_positions, y_means, y_lowers, y_uppers = [], [], [], []
...
#     plt.ylabel(f'Average end-to-end delay [{time_label}]', fontsize=20)
...
def plot_average_delay__capacity_priority(
    ...
    """
    Plots the average end-to-end delay versus capacity and priority for different traffic demands.
    ...
    Args:
        ...
        plot_priority_our_model (bool): Whether to plot delay by priority for 'our model'.
        plot_priority_capacity (bool): Whether to plot delay by priority for 'capacity'.
        plot_overall_our_model (bool): Whether to plot overall delay for 'our model'.
        plot_overall_capacity (bool): Whether to plot overall delay for 'capacity'.
    ...
    if plot_overall_our_model:
        ...
        for demand, mean, ci_lower, ci_upper in sorted(data['overall']['our_model']):
            x = demand_to_x[demand]
            x_positions.append(x)
            # Convert delay values based on time_unit (delays from SimPy are in seconds)
            if time_unit == 'ms':
                y_means.append(mean * 1000)
                y_lowers.append(ci_lower * 1000)
                y_uppers.append(ci_upper * 1000)
            else:
                y_means.append(mean)
                y_lowers.append(ci_lower)
                y_uppers.append(ci_upper)
...
def plot_average_PLI__capacity_priority(
    ...
    """
    Plots the average Packet Loss Indicator (PLI) versus capacity and priority for different traffic demands.

    Args:
        all_results (dict): A dictionary containing the simulation results.
        time_unit (str): The unit of time for the x-axis label ('s' for seconds, 'ms' for milliseconds).
        plot_priority_our_model (bool): Whether to plot PLI by priority for 'our model'.
        plot_priority_capacity (bool): Whether to plot PLI by priority for 'capacity'.
        plot_overall_our_model (bool): Whether to plot overall PLI for 'our model'.
        plot_overall_capacity (bool): Whether to plot overall PLI for 'capacity'.
        results_dir (str): The directory to save the plot.
