"""
This class represents a Static Priority (SP) server which is designed to manage
and simulate the distribution of network traffic based on predefined priorities.
In SP scheduling, packets from higher-priority queues are always processed before
packets from lower-priority queues, ensuring strict priority order.

The server can operate with or without buffers, and supports both flow-based
and class-based priority mechanisms. It provides various methods to update
internal statistics, manage packet flow, and interact with upstream and
...
class SPServer:
    """
    A class to represent a Static Priority (SP) server.
    ...
    flow_classes: function, optional
    ...
    zero_downstream_buffer: bool, optional
    ...
    def __init__(self,
        ...
        self.priorities_list = sorted(self.prio_queue_count, reverse=False)
    ...
    def update_stats(self, packet):
        ...
        if self.debug:
            print(
                f"Sent out packet {packet.id} from flow {packet.flow_id} "
    ...
    def put(self, packet, upstream_update=None, upstream_store=None):
        ...
        if self.debug:
            print(
                ...
                .format(self.env.now, packet.id, packet.flow_id,
