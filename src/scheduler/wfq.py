"""
This class represents a WFQ server which is designed to manage and simulate
the distribution of network traffic in a fair manner based on specified weights.
WFQ ensures that flows or classes with higher weights get more bandwidth
relative to those with lower weights.

The server can operate with or without buffers, and supports both flow-based
and class-based queueing mechanisms. It provides various methods to update
...
from utils import taggedstore


class WFQServer:
    """
    A class to represent a Weighted Fair Queueing (WFQ) server.
    ...
    def update_stats(self, packet):
        ...
        if self.debug:
            print(
                f"Sent Packet {packet.id} from flow {packet.flow_id} "
